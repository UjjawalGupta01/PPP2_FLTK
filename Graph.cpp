...
namespace Graph_lib {
	...
	void Polygon::add(Point p)
	{
		...
		if (1 < np) {	// check that thenew line isn't parallel to the previous one
			if (p == point(np - 1)) std::cerr << ("polygon point equal to previous point");
			bool parallel;
			line_intersect(point(np - 1), p, point(np - 2), point(np - 1), parallel);
			if (parallel)
				std::cerr << ("two polygon points lie in a straight line");
		}

		for (int i = 1; i < np - 1; ++i) {	// check that new segment doesn't interset and old point
			Point ignore(0, 0);
			if (line_segment_intersect(point(np - 1), p, point(i - 1), point(i), ignore))
				std::cerr << ("intersect in polygon");
		}
	...
	{
		if (number_of_points() % 2 == 1) std::cerr << ("odd number of points in set of lines");
		if (color().visibility())
	...
	{
		if (r2 - r1 <= 0) std::cerr << ("bad graphing range");
		if (count <= 0) std::cerr << ("non-positive graphing count");
	...
	Axis::Axis(Orientation d, Point xy, int length, int n, std::string lab)
		:label(Point(0, 0), lab)
	{
		if (length < 0) std::cerr << ("bad axis length");
		switch (d) {
		case Axis::x:
		...
		case Axis::z:
			std::cerr << ("z axis not implemented");
		}
	}
