...
class Widget
{
    ...
    virtual void attach(
        Window&) = 0;    // each Widgit define at least one action for a window
    ...
    /*
    Widget(const Widget& a) :loc(a.loc) { error("attempt to copy Widget by
    constructor"); } Widget& operator=(const Widget& a)
    {
...
struct In_box : Widget {
    In_box(Point xy, int w, int h, const std::string& s): Widget(xy, w, h, s, 0)
    {
    }
    int get_int();
