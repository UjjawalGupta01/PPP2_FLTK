...
class Shape    // deals with color and style, and holds sequence of lines
{
    ...
    Shape(
        std::initializer_list<Point> lst);    // add() the Points to this Shape
...
struct Rectangle : Shape {
    ...
    {
        if (h <= 0 || w <= 0) {
            std::cerr << ("Bad rectangle: non-positive side");
        }
        add(xy);
    }
    ...
    {
        if (h <= 0 || w <= 0) {
            std::cerr << ("Bad rectangle: first point is not top left");
        }
        add(x);
    }
...
struct Lines : Shape {    // indepentdent lines
    ...
    {
        if (lst.size() % 2) {
            std::cerr << ("odd number of points for Lines");
        }
    }
    void draw_lines() const;
...
struct Text : Shape {
    ...
    Text(Point x, const std::string& s): lab {s}
    {
        add(x);
    }
