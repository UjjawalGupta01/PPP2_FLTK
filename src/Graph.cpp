...
void Polygon::add(Point p)
{
    ...
    if (1 <
        np) {    // check that the new line isn't parallel to the previous one
        if (p == point(np - 1))
            std::cerr << ("polygon point equal to previous point");
        bool parallel;
...
{
    if (r2 - r1 <= 0)
        std::cerr << ("bad graphing range");
    if (count <= 0)
        std::cerr << ("non-positive graphing count");
